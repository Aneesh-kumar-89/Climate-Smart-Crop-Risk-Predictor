...
def main():
    ...
    with st.sidebar:
        ...
        st.write("1. Get real-time weather or input manually")
        st.write("2. AI analyzes climate stress factors")
        st.write("3. Get risk level & recommendations")
        st.write("4. Take preventive action")
        
        st.header("🔑 API Setup")
        api_key = st.text_input("OpenWeatherMap API Key", type="password", help="Optional: Enter your API key for real-time weather")
        if api_key:
    ...
    location_method = st.radio(
        ...
        ["🎯 Auto-detect my location (IP-based)", "🌐 Use precise GPS location", "📝 Enter city manually"],
        help="Auto-detect uses your IP address, GPS uses browser geolocation for precise coordinates"
    )
    
    if location_method.startswith("🎯"):
        # Auto-detect location using IP
        auto_detect_col1, auto_detect_col2 = st.columns([1, 1])
        
        with auto_detect_col1:
            auto_detect = st.button("📍 Detect My Location", type="primary")
        
        with auto_detect_col2:
            if st.session_state.get('detected_location'):
                loc = st.session_state.detected_location
                st.success(f"📍 Detected: {loc.get('city', 'Unknown')}, {loc.get('country_name', 'Unknown')}")
        
        if auto_detect:
            with st.spinner("🔍 Detecting your location..."):
                detected_location = weather_processor.get_coordinates_by_ip()
                if detected_location:
                    st.session_state.detected_location = detected_location
                    st.success(f"✅ Location detected: {detected_location['city']}, {detected_location['country_name']}")
                    st.rerun()
                else:
        ...
        if st.session_state.get('detected_location'):
            city_name = st.session_state.detected_location['city']
            country_code = st.session_state.detected_location['country']
            fetch_weather = st.button("🌤️ Get Weather for My Location", type="secondary")
        else:
            city_name = "Mumbai"
            country_code = "IN"
            fetch_weather = False
            st.info("👆 Click 'Detect My Location' to automatically find your city")
    
    elif location_method.startswith("🌐"):
        # GPS-based location detection
        st.info("🌐 **GPS Location Detection**")
        st.markdown("""
        This feature uses your browser's geolocation API for precise coordinates.
        You'll need to allow location access when prompted by your browser.
        """)
        
        # JavaScript component for geolocation
        gps_col1, gps_col2 = st.columns([1, 1])
        
        with gps_col1:
            if st.button("🛰️ Get GPS Location", type="primary"):
                ...
                function showPosition(position) {
                    ...
                    // Store coordinates in session state via Streamlit
        ...
        with gps_col2:
            if st.session_state.get('gps_location'):
                gps_loc = st.session_state.gps_location
                st.success(f"📍 GPS: {gps_loc.get('lat', 0):.4f}, {gps_loc.get('lon', 0):.4f}")
        ...
        with coord_col1:
            manual_lat = st.number_input("Latitude", value=19.0760, format="%.4f", help="Your latitude coordinate")
        
        with coord_col2:
            manual_lon = st.number_input("Longitude", value=72.8777, format="%.4f", help="Your longitude coordinate")
        
        with coord_col3:
            use_coords = st.button("🌤️ Get Weather by Coordinates", type="secondary")
        
        if use_coords:
            with st.spinner(f"Fetching weather for coordinates {manual_lat:.4f}, {manual_lon:.4f}..."):
                # Get weather directly by coordinates
                weather_data = weather_processor.fetch_current_weather(manual_lat, manual_lon)
                if weather_data:
                    real_time_weather = weather_processor.process_current_weather_data(weather_data)
                    use_real_time = True
                    city_name = f"Lat: {manual_lat:.4f}"
                    country_code = f"Lon: {manual_lon:.4f}"
                    fetch_weather = False  # Already fetched
                else:
                    st.error("❌ Could not fetch weather data for these coordinates")
                    city_name = "Mumbai"
                    country_code = "IN"
                    fetch_weather = False
        else:
    ...
    if fetch_weather and city_name:
        with st.spinner(f"Fetching real-time weather for {city_name}..."):
            ...
            if real_time_weather and real_time_weather.get('location') != 'Demo Location':
                ...
                # Display current weather in a nice card
                st.markdown('<div class="weather-card">', unsafe_allow_html=True)
                st.subheader(f"🌤️ Current Weather in {real_time_weather['location']}")
                ...
                st.error("❌ Could not fetch weather data. Please check city name, API key, or try again later.")
    ...
    **Data Sources**: Real-time weather from OpenWeatherMap API. Synthetic training data for demonstration.
    """)
